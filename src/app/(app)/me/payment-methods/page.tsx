...
import {
  ...
  DialogDescription,
  DialogTrigger,
} from "@/components/ui/dialog"; // DialogFooter not used directly here
import {
  UserPaymentMethodForm,
  UserPaymentMethodFormValues,
} from "@/components/features/me/UserPaymentMethodForm";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function UserPaymentMethodsPage() {
  ...
  const fetchUserPaymentMethods = useCallback(async () => {
    ...
    try {
      const methods = await fetchApi<PaymentMethod[]>(
      ...
      if (apiError.status === 501) {
        toast({
          ...
          description:
            "Listing your payment methods is currently unavailable. (Stubbed 501)",
          variant: "info",
          duration: 5000,
        });
        setPaymentMethods([]);
  ...
  const handleSaveMethod = async (formData: UserPaymentMethodFormValues) => {
    ...
    try {
      if (editingMethod && editingMethod.id) {
        ...
        await fetchApi<PaymentMethod>(
          `/api/v1/me/payment-methods/${editingMethod.id}`,
          "PUT",
          payload,
        );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await fetchApi(
        `/api/v1/me/payment-methods/${showDeleteConfirm.id}`,
        "DELETE",
      );
      ...
      if (apiError.status === 501) {
        toast({
          title: "Feature Coming Soon",
          description:
            "Deleting payment methods is not yet implemented. (Stubbed 501)",
          variant: "info",
          duration: 5000,
        });
        ...
        setPaymentMethods((prev) =>
          prev.filter((m) => m.id !== showDeleteConfirm.id),
        );
        setShowDeleteConfirm(null);
      } else {
        toast({
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <header className="py-4">
        <h1 className="text-3xl font-bold flex items-center">
          <CreditCard className="mr-3 h-8 w-8 text-primary" /> My Payment
          Methods
        </h1>
      ...
      {error &&
        !isLoading && ( // Only show standalone error if not loading and error exists
          <Card className="bg-destructive/10 border-destructive">
            ...
            <CardContent>
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={fetchUserPaymentMethods}
                className="mt-3"
      ...
      <Card>
        <CardHeader className="flex flex-row justify-between items-center">
          <div>
            ...
            <CardDescription>
              View, add, or manage your payment methods.
            </CardDescription>
          </div>
          <Button
          ...
          {paymentMethods.length > 0 && (
            <ul className="space-y-3">
              {paymentMethods.map((method) => {
                const isOnlyPrimary =
                  method.isPrimary &&
                  paymentMethods.filter((pm) => pm.isPrimary).length <= 1;
                return (
                  <li
                    key={method.id}
                  ...
                  >
                    ...
                    <div className="space-x-2">
                      <Button
                        variant="outline"
                        size="icon"
                        onClick={() => handleOpenFormDialog(method)}
                        disabled={isLoading || isSubmitting || isDeleting}
                      >
        ...
        {paymentMethods.length > 0 && (
          <CardFooter className="text-sm text-muted-foreground">
            Note: To change your primary payment method, edit the desired method
            and set it as primary. Deleting the only primary method is not
            allowed.
          </CardFooter>
        )}
      </Card>
      ...
      <Dialog
        open={isFormOpen}
        onOpenChange={(isOpen) => {
          if (!isSubmitting && !isDeleting) setIsFormOpen(isOpen);
        }}
      >
        <DialogContent className="sm:max-w-[425px]">
          ...
          <UserPaymentMethodForm
            initialData={
              editingMethod
                ? { ...editingMethod, id: String(editingMethod.id) }
                : undefined
            }
            onSave={handleSaveMethod}
            isSaving={isSubmitting}
            onCancel={() => {
              if (!isSubmitting && !isDeleting) setIsFormOpen(false);
            }}
          />
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog
        ...
        onOpenChange={(isOpen) => {
          if (!isDeleting)
            setShowDeleteConfirm(isOpen ? showDeleteConfirm : null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Details:{" "}
              {showDeleteConfirm?.type === "Credit Card"
                ? `**** ${showDeleteConfirm?.last4}`
